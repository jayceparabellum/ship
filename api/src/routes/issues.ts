...
function getAuthContext(req: Request): { userId: string; workspaceId: string } {
  if (!req.userId || !req.workspaceId) {
    throw new Error('Authenticated issue route missing request context');
  }
  ...
  try {
    ...
    if (isClosingIssue && wasNotClosed) {
      ...
      if (props.source === 'action_items') {
        const assigneeId = typeof props.assignee_id === 'string' ? props.assignee_id : userId;
